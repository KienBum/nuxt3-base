<template>
  <div class="file-upload">
    <label class="file-input">
      ...
      {{ label }}
      <input :multiple="multiple" type="file" ref="inputFile" name="inputFile" :accept="accept"
    ...
    <template v-if="files.length > 0">
      <template v-if="onThumbnail">
        <div class="thumbnail-preview-container">
          <div v-for="(file, index) in files" :key="index" class="thumb-item"
            :class="isUpload ? 'thumb-item-uploaded' : 'thumb-item-error'">
            <div class="thumb-img">
              <img v-if="file.isImage" :src="file.downloadUrl" alt="preview upload files">
              <svg class="file-img" v-else xmlns="http://www.w3.org/2000/svg" width="120" height="120"
                viewBox="0 0 24 24">
            ...
            <div class="thumb-detail">
              <div class="thumb-size">
                <span><strong>{{ `${file.fileSize}` }}</strong>{{ ` MB` }}</span>
              </div>
              <div class="thumb-filename">
                <span title="Click to download file" @click="downloadFile(file)">{{ file.fileFullName }}</span>
              </div>
              <div class="thumb-action">
                <div class="thumb-delete" title="Delete file" @click="handleDeleteFile(file, index)">
      ...
      <template v-else>
        <div class="file-preview-container">
          <div v-for="(file, index) in files" :key="index" class="file-preview">
            <img v-if="file.isImage" class="file-img" :src="file.downloadUrl" alt="preview upload files">
            <svg class="file-img" v-else xmlns="http://www.w3.org/2000/svg" width="55" height="55" viewBox="0 0 24 24">
              <path fill="currentColor"
                d="M14 4H6v16h12V8h-4V4zm2 7v4c0 2.21-1.79 4-4 4s-4-1.79-4-4V9.5a2.5 2.5 0 0 1 5 0V15h-2V9.5c0-.28-.22-.5-.5-.5s-.5.22-.5.5V15c0 1.1.9 2 2 2s2-.9 2-2v-4h2z"
                opacity=".3" />
              <path fill="currentColor"
            ...
            <a @click="downloadFile(file)" class="file-name" title="Click to download file">{{ `${file.fileFullName} -
                          ${file.fileSize}MB` }}</a>
...
const props = defineProps({
  ...
  isPreview: { // use for preview files after upload
    type: Boolean,
    default: false,
    required: false,
  },
  onThumbnail: { // to use this props, make sure isPreview = true 
    type: Boolean,
    default: false,
    required: false,
  },
      ...
      useAPI({
        ...
        .then((value: any) => {
          ...
          const file: any = {
            ...
            isImage: ["jpg", "jpeg", "png", "gif"].includes(fileName.split(".").pop()),
            downloadUrl: URL.createObjectURL(new Blob([response], { type: response.type }))
          }
...
const downloadFile = async (file: any) => { // func download file
  ...
  const FILE = window.URL.createObjectURL(new Blob([response.value], { type: response.value.type }));
...
const handleFileChange = (e: any) => { // func handle when user selects file
...
const uploadToServerMul = async (listFiles: any) => { // func upload files after user selected its
...
const isFileValid = (file: any) => { // func check valid files
...
const isFileSizeValid = (file: any) => {
  ...
  errors.value = [...errors.value, `File ${file.fileName} size should be less than ${props.maxSize} MB`];
}

const isFileTypeValid = (file: any) => {
  if (
    props.acceptExtension === "*" ||
    props.acceptExtension.replace(/\s/g, '').split(",").includes(file.fileExtention)
  ) {
    return;
  }
  errors.value = [...errors.value, `File ${file.fileName} type should be ${props.acceptExtension}`];
}
</script>
