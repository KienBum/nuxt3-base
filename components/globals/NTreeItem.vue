<template>
  <li class="item" :class="{ 'tree-node' : isLine}">
    <div class="title" :class="isOpen ? 'bold': ''">
      <NIcon
        v-if="isFolder"
        :name="isOpen ? 'material-symbols:arrow-drop-down-rounded' : 'material-symbols:arrow-right-rounded'"
        size="22"
        @click="toggle"
      ></NIcon>
      <NCheck
    ...
    <Transition name="append">
        <ul class="child-item" :class="{ 'tree-node-child' : isLine}" v-show="isOpen" v-if="isFolder">
          <NTreeItem
