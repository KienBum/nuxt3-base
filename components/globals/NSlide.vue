...
const ifArrayOfString = (arr: string[]) => {
  const isArray = Array.isArray(props.imgArr);
  const notEmpty = props.imgArr.length > 0;
  const invalidValue = props.imgArr.find((item) => typeof item !== "string");

  if (isArray && notEmpty && !invalidValue) return true;
  return false;
};

const validArray = ifArrayOfString(props.imgArr);

const resolveImages = (arr: string[]) => arr.map((img: string) => img);
